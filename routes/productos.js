...
router.post('/insertar', function (req, res, next) {
    ...
    if (!nombre || !password || !admin) {
        return res.status(500).send("No se inserto correctamente");
    }
    // Si todo va bien, seguimos
    productosModel
        .insertar(nombre, password, admin)
...
router.post('/actualizar/', function (req, res, next) {
    ...
    // const nombre = req.body.nombre;
    // const precio = req.body.precio;
    const { id, nombre, password, admin } = req.body;
    if (!nombre || !password || !admin || !id) {
        return res.status(500).send("No hay suficientes datos");
    }
    // Si todo va bien, seguimos
    productosModel
        .actualizar(id, nombre, password, admin)
        .then(() => {
            res.redirect("/productos");
        })
        .catch(err => {
